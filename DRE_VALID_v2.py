   ...
   def solver_sinais_balanco(self, df, id_resultado, id_parcela1, id_parcela2):
    ...
    if solucoes_validas:
        ...
        if len(valores_id_pri) > 0:
            indices_id_pri = df[df[self.column_account] == id_dep_pri].index
            for i, idx in enumerate(indices_id_pri):
                df_resultado.loc[idx, 'sinal'] = sinais[i]
        
        # Para ID secundário
        if len(valores_id_sec) > 0:
            indices_id_sec = df[df[self.column_account] == id_dep_sec].index
        ...
        df_resultado['valor_com_sinal'] = df_resultado[self.column_value] * df_resultado['sinal']
